...
def compute_attribution_saliency_maps_intergrated(text_char,
  ...
  interpolated_char_inputs = interpolate_inputs(baseline_char_emb, text_char_emb, steps)
  interpolated_word_inputs = interpolate_inputs(baseline_word_emb, text_word_emb, steps)
  ...
  avg_gradient_subregion_char = accumulated_gradient_subregion_char / steps
  avg_gradient_subregion_word = accumulated_gradient_subregion_word / steps
  avg_gradient_date_char = accumulated_gradient_date_char / steps
  avg_gradient_date_word = accumulated_gradient_date_word / steps
...
def compute_attribution_saliency_maps_intergrated_centroid(text_char,
                                      text_word,
                                      text_len,
                                      padding,
                                      forward,
    ...
    # Compute sentence-level centroid baseline (mean across entire sentence)
    centroid_char_emb = jnp.mean(text_char_emb, axis=(0, 1), keepdims=True)
    centroid_word_emb = jnp.mean(text_word_emb, axis=(0, 1), keepdims=True)

    # Generate interpolated inputs between centroid and actual input
    interpolated_char_inputs = interpolate_inputs(centroid_char_emb, text_char_emb, steps)
    interpolated_word_inputs = interpolate_inputs(centroid_word_emb, text_word_emb, steps)
    ...
    # Normalize accumulated gradients over all steps
    avg_gradient_subregion_char = accumulated_gradient_subregion_char / steps
    avg_gradient_subregion_word = accumulated_gradient_subregion_word / steps
    avg_gradient_date_char = accumulated_gradient_date_char / steps
    avg_gradient_date_word = accumulated_gradient_date_word / steps
    ...
    integrated_gradients_date_char = (text_char_emb - centroid_char_emb) * avg_gradient_date_char
    integrated_gradients_date_word = (text_word_emb - centroid_word_emb) * avg_gradient_date_word
...
def interpolate_inputs(baseline, input_emb, steps=50):
    """Generate interpolated inputs from the baseline (sentence-level centroid) to the actual input."""
    alphas = np.linspace(0, 1, steps)
    interpolated_inputs = [(1 - alpha) * baseline + alpha * input_emb for alpha in alphas]
    return interpolated_inputs
